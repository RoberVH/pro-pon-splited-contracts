...
describe('********************** Company1.js ********************** \nCompany Creation', function () {
  it('1 Should have assigned proponLogicContract as owner of proponDataContract', async function () {
    const { proponDataContract, proponLogicContract } = await loadFixture(
      deployPropon
    )
  ...
  it('4 Should assign HardHat owner  address as owner of proponLogicContract', async function () {
    const { proponDataContract, proponLogicContract, owner, addr1 } =
  ...
  it('6 Should allow transferring Data Contract management to other address', async function () {
    const { proponDataContract, proponLogicContract, owner, addr1, addr2 } =
      await loadFixture(deployPropon)
  ...
  it('7 After changing Owner of Data Contract cannot call an Owner method', async function () {
    const { proponDataContract, proponLogicContract, owner, addr1, addr2 } =
      await loadFixture(deployPropon)
    // transfer Management from owner to addr2 address. Notice this doesn't make sense as addr in not a proponlogic contract but it's enough to proof correct transfering
    // this will come to play in case Logic changes, the Data contract will remain with a new manager Logic Contract
  ...
  it('8 After changing Owner of Data Contract and changing again should allow  call an Owner method', async function () {
    const {
      proponDataContract,
      proponLogicContract,
      owner,
    ...
    // create another instance of propon Logic Contract to receive ownership of Data contract
    const proponLogic2 = await ethers.getContractFactory('pro_ponLogic')
   ...
   // first company created when Data contract owner is still authorized Manager account (owner account from hardhat) 
    ...
    let c1 = await proponDataContract.getCompanyId(addr1.address) // first company created
    expect(c1).to.equal(test_pro_pon1.id)
    c1 = await proponDataContract.getCompanyId(addr2.address) // second company created
    expect(c1).to.equal(test_pro_pon2.id)
    c1 = await proponDataContract.getCompanyId(owner.address)  // third company created
    expect(c1).to.equal(test_pro_pon3.id)
  })
})

describe('********************** Company1.js ********************** \nCompany Creation', function () {
  it('2 Should create a company', async function () {
    const { proponDataContract, proponLogicContract, owner } =
      await loadFixture(deployPropon)
...
describe('Reusing Companies Ids', function () {
  ...
  it('2 Should reject create company with taken Id from different address', async function () {
    ...
    await expect(
      proponLogicContract.connect(addr1).createCompany(
...
describe('Test destroying contract', function () {
  it('1 Should denied destroying Data contract from non-manager request', async function () {
    ...
    await expect(
      proponDataContract.connect(addr1).destroy()
  ...
  it('3 Should denied non owner to destroy Logic contract', async function () {
    ...
    expect(companyId).to.equal(test_pro_pon1.id)
    // Alright contract exist, let's destroy it
    expect(proponLogicContract.connect(addr1).destroy()).to.be.revertedWith(
      'Only_owner_allowed'
    )
  ...
  it('5 Should denied withdraw contract funds to no owner of contract', async function () {
    ...
    await expect(
      proponLogicContract.connect(addr1).withdraw()
    ).to.be.revertedWith('Only_owner_allowed')
...
describe('Test setting/getting prices on Manager Contract', function () {
  it('1 Should allow manager address set CreateCompanyPrice', async function () {
    ...
    const price = 20.504
    let txn = await proponDataContract.setCreateCompanyPrice(
  ...
  it('2 Should deny no  manager address set CreateCompanyPrice', async function () {
    ...
    expect(
      proponDataContract
        .connect(addr3)
        .setCreateCompanyPrice(ethers.utils.parseUnits(price.toString()))
    ).to.be.revertedWith('only_manager_allowed')
  ...
  it('3 Should allow manager set CreateInvitationRfpPrice', async function () {
    ...
    // set the price in Matics that are converted to wei. Set price to 20 matics
    const price = 0.504
    let txn = await proponDataContract.setCreateInvitationRfpPrice(
      ethers.utils.parseUnits(price.toString())
    )
  ...
  it('4 Should allow a newly set manager set RegisterOpenRfpPrice', async function () {
    const { proponDataContract, owner, addr1, addr3 } = await loadFixture(
      deployPropon
    )
    ...
    let txn = await proponDataContract
      .connect(addr3)
      .setRegisterOpenRfpPrice(ethers.utils.parseUnits(price.toString()))
  ...
  it('5 Should allow owner set CREATE_OPEN_RFP_PRICE', async function () {
    ...
    // set the price in Matics that are converted to wei. Set price to 20 matics
    const price = 10.504
    let txn = await proponDataContract.setCreateOpenRfpPrice(
      ethers.utils.parseUnits(price.toString())
    )
  ...
  it('6 Should allow owner set maxguest to open and invitation contests', async function () {
    ...
    expect(await proponDataContract.MAX_GUEST_OPEN_TENDER()).to.be.equal(guests)
    await proponDataContract.setMaxGuestInvitationTender(guests)
    expect(await proponDataContract.MAX_GUEST_INVITATION_TENDER()).to.be.equal(
      guests
    )
  })
  ...
  it('7 Should allow get owner of ProponLogic contract', async function () {
    const { proponDataContract, proponLogicContract, owner, addr1 } =
      await loadFixture(deployPropon)
    expect(await proponLogicContract.getOwner()).to.be.equal(owner.address)
  })
  ...
  it('8 Should allow  owner of ProponLogic contract set another owner', async function () {
    const { proponDataContract, proponLogicContract, owner, addr1 } =
      await loadFixture(deployPropon)
    await proponLogicContract.setOwner(addr1.address)
    expect(await proponLogicContract.getOwner()).to.be.equal(addr1.address)
  })
  ...
  it('9 Should allow  current owner of ProponLogic contract to withdraw balance', async function () {
    ...
    expect(companyId).to.equal('')
    await proponLogicContract.setOwner(addr1.address)
    let contratoantes = await ethers.provider.getBalance(
