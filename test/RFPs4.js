...
const createRFP = async (proponContract, address, RFPNameIdx, ContestType, ItemsList, value) => {
  ...
  let endReceiving=convertDatesAgo(-2)  // Two days from now
  let endDate=convertDatesAgo(-4)  // Four days from now
  ...
  async function deployProponandCreateCompanies() {
    ...
    return { proponDataContract,proponLogicContract,  owner, addr1, addr2, addr3, addr4, addr5,addr6, addr7,addr8 };
  }  
  ...
  describe("********************** RFPs4.js ********************** \nRecord participant invitation companies to RFP ", function () {
    ...
    it("0 Should refuse creating RFP beggining more than one hour", async function () {
      ...
      let openDate=convertDatesAgo(0) - (120 * 60) // two hours  ago
      let endReceiving=convertDatesAgo(-3) // 3 days from now
      let endDate=convertDatesAgo(-5)  // 5 days from now
      expect(proponLogicContract.connect(addr2).createRFP(IdofRFPArray[1],nameRfp[1],rfpWebLink[1],openDate,endReceiving,
        endDate,ContestType.OPEN,listItems3, {value: ethers.utils.parseEther('0.0001')})).to.be.revertedWith('opendate_behind_today')
      // expect(proponLogicContract.connect(addr2).createRFP(IdofRFPArray[2],nameRfp[2],rfpWebLink[2],openDate,endReceiving,endDate,ContestType.INVITATION,listItems2,
    ...
    it("0 Should record invitation to three companies (invitation contest) of addr2 company RFP", async function () {
      const { proponDataContract,  proponLogicContract,  owner, addr1, addr2, addr3 } = await loadFixture(deployProponandCreateCompanies);
      // create 1 Open & 1 Invitation RFPs  for company addr2
      let openDate=convertDatesAgo(0)  // now hours  ago
      let endReceiving=convertDatesAgo(-3) // 3 days from now
      let endDate=convertDatesAgo(-5)  // 5 days from now
      await proponLogicContract.connect(addr2).createRFP(IdofRFPArray[1],nameRfp[1],rfpWebLink[1],openDate,endReceiving,
        endDate,ContestType.OPEN,listItems3, {value: ethers.utils.parseEther('0.0001')}
      ) 
      await proponLogicContract.connect(addr2).createRFP(IdofRFPArray[2],nameRfp[2],rfpWebLink[2],openDate,endReceiving,endDate,ContestType.INVITATION,listItems2,
            {value: ethers.utils.parseEther('0.0002')}
      )       
      // get addr2 company record
      let company= await proponDataContract.getCompany(addr2.address)
    ...
    it("3 Should ignore invitation for two companies already invited", async function () {
      const { proponDataContract,  proponLogicContract, owner, addr1, addr2, addr3, addr4, addr5 } = await loadFixture(deployProponandCreateCompanies);
      ...
      let RFPidx = await company.company_RFPs[1]  
      await proponLogicContract.inviteCompaniestoRFP(  // invite first set of companies (addr1, addr3 & addr4)
      RFPidx,
      test_pro_pon1.id,
      [addr1.address,addr3.address] )
      await proponLogicContract.inviteCompaniestoRFP(  // invite second set of companies
      RFPidx,
      test_pro_pon1.id,
    ...
    it("4 Should ignore invitation for a companie already invited and owner of rfp", async function () {
      ...
      let company= await proponDataContract.getCompany(owner.address)      // retrieve second RFP of this company
      let RFPidx = await company.company_RFPs[1]  
      await proponLogicContract.inviteCompaniestoRFP(  // invite first set of companies
      RFPidx,
      test_pro_pon1.id,
      [addr1.address,addr3.address] )
      await proponLogicContract.inviteCompaniestoRFP(  // invite second set of companies
    ...
    it("5 Should reject not RFP issuer invitation when company Id is wrong", async function () {
      ...
      expect(proponLogicContract.inviteCompaniestoRFP(    // try to invite addr2 company from owner account
        RFPidx,               
    ...
    it("7 Should register MAX_GUEST_INVITATION_TENDER companies to invitation Contest in different loads", async function () {
      const { proponDataContract,  proponLogicContract, owner, addr1,addr2, addr3, addr4,addr5, addr6, addr7, addr8 } = await loadFixture(deployProponandCreateCompanies);
      // get first company record
      // create RFPs
      await createRFP(proponLogicContract, addr1, 3, ContestType.INVITATION, listItems3, '0.0002')
      await createRFP(proponLogicContract, addr1, 0, ContestType.INVITATION, items, '0.0002')
  ...
  describe("Record participant registration to OPEN and INVITATION RFPs ", function () {
    ...
    it("13 Should reject more companies than max limit allowed register to open tender", async function () {
      ...
      it("14 Should reject adding owner of invitation RFP to own rfp", async function () {
        const { proponDataContract,  proponLogicContract,  owner, addr1, addr2, addr3, addr4 } = await loadFixture(deployProponandCreateCompanies);
          // crete RFP
          await createRFP(proponLogicContract, owner, 2, ContestType.OPEN, listItems2, '0.0002') 
          await createRFP(proponLogicContract, owner, 1, ContestType.INVITATION, listItems1, '0.0002') 
        // get RFP owner company record
        ...
        // retrieve second RFP of this company
        let RFPidx = await company.company_RFPs[1]  // Second RFP from owner company
        await proponLogicContract.connect(owner).inviteCompaniestoRFP(    // invite correctly 2 companies
          RFPidx,
          test_pro_pon1.id,   // Company Id of owner
          [addr2.address, addr1.address]       // adds addr1 & addr2 companies total of 2
          )
          await proponLogicContract.connect(owner).inviteCompaniestoRFP(    // trying to correct addr3 and incorrect invite  owner
          RFPidx,
          test_pro_pon1.id,   // Company Id of owner
