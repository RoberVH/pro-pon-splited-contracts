...
const main = async () => {
  ...
  // get Balance
  const proponLogicBal = await alchemyprovider.getBalance(
    CONTRACT_ADDRESS_LOGIC
  )
  console.log('Propon_Logic Balance:', ownerLogic)
  console.log(
    `The balance of Propon_Logic (${CONTRACT_ADDRESS_LOGIC}) es ${ethers.utils.formatEther(
      proponLogicBal
    )} MATICS`
  )
  console.log('--------------------------------------------------------\n')
