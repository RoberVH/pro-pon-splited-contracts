...
const main = async () => {
    ...
    const[owner, add2, addr3] = await ethers.getSigners();
  // 1rst creation ---------------------------------------------------
  ...
  console.log('\n 3rd creation it must failed, same owner address-----------------------')
  txn = await pro_ponContract.createCompany(
      test_pro_pon2.id,
      test_pro_pon2.name,
      {value: ethers.utils.parseEther('0.0001')})
  // Wait for it to be mined.
